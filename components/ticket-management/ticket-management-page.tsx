...
const StatCard = memo(function StatCard({ 
  ...
  const bgClass = colorClass.replace('border-l-', 'bg-').replace('-500', '-50/50')
  const darkBgClass = colorClass.replace('border-l-', 'dark:from-').replace('-500', '-950/30')
  const textClass = colorClass.replace('border-l-', 'text-').replace('-500', '-700')
  const darkTextClass = colorClass.replace('border-l-', 'dark:text-').replace('-500', '-300')
  const iconBgClass = colorClass.replace('border-l-', 'bg-').replace('-500', '-500/15')
  
  return (
    <Card className={`group relative overflow-hidden hover:shadow-md transition-all duration-300 border-l-3 ${colorClass} bg-gradient-to-br from-${colorClass.split('-')[1]}-50/50 to-white ${darkBgClass} dark:to-background`}>
      <div className={`absolute top-0 right-0 w-12 h-12 ${iconBgClass.replace('-500/15', '-500/10')} rounded-full -mr-6 -mt-6 group-hover:scale-150 transition-transform duration-500`} />
      <CardContent className="p-3">
        <div className="flex items-center justify-between">
          <div className="space-y-0.5">
            <p className="text-sm font-bold text-gray-700 dark:text-gray-200">{title}</p>
            <p className={`text-2xl font-bold ${textClass} ${darkTextClass}`}>{value}</p>
            <p className="text-[10px] font-medium text-gray-500 dark:text-gray-400">{subtitle}</p>
          </div>
          <div className={`p-2 ${iconBgClass} rounded-lg`}>
            <Icon className={`h-4 w-4 ${textClass} ${darkTextClass}`} />
          </div>
        </div>
      </CardContent>
    </Card>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {statsLoading ? (
          ...
          <>
            {[1, 2, 3, 4].map((i) => (
              <Card key={i} className="border-l-3 border-l-gray-200">
                <CardContent className="p-3">
                  <div className="flex items-center justify-between">
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">{stats?.myTickets ?? 0}</p>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-2xl font-bold text-orange-700 dark:text-orange-300">{stats?.pending ?? 0}</p>
              ...
              <CardContent className="p-3">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-2xl font-bold text-red-700 dark:text-red-300">{stats?.slaAtRisk ?? 0}</p>
